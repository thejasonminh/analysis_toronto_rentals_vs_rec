...
[^*]: Code and data supporting this analysis is available at: https://github.com/thejasonminh/analysis_toronto_rentals_vs_rec
...
library("tidyverse")
cleaned_apartment_data |> ggplot() + geom_col(aes(x=year_built, y=Count)) + theme_minimal() + labs(x = "Year Built", y = "Number Built")
```

From 2015 to 2019, there were 65 apartment buildings built @odtcitation]. However, when the COVID-19 pandemic hit in 2020 the amount of apartment units built decreased heavily, with only 4 buildings built in 2020, 6 buildings built in 2021 and 3 buildings built in 2022.
...
library("tidyverse")
cleaned_bike_data |> ggplot() + geom_col(aes(x=Year_Installed, y=Count)) + theme_minimal() + labs(x = "Year Installed", y = "Number Installed")
```
...
cleaned_applications_data |> ggplot() + geom_col(aes(x=status, y=Count)) + theme_minimal() + labs(x = "Application Status", y = "Number of Applications")
```
