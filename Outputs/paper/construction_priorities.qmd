...
library("tidyverse")
cleaned_apartment_data |> ggplot() + geom_col(aes(x=year_built, y=Count)) + theme_minimal()
```
...
library("tidyverse")
cleaned_bike_data |> ggplot() + geom_col(aes(x=Year_Installed, y=Count)) + theme_minimal()
```
...
cleaned_applications_data |> ggplot() + geom_col(aes(x=status, y=Count)) + theme_minimal()
```
...
TALK ABOUT APARTMENT BUILDINGS VS CYCLING PATHS HERE, TRY TO DO 1 OR 2 PARAGRAPHS. TALK ABOUT PRIORITY SHIFT
